...
/* Field for storing objects like this -- rates{key:{..., name="Andorra"}} */
formTagEvent.countries = [];

/* Field for storing -- selected country by ID*/
formTagEvent.selectCountry = null;

/* Simple facades methods for getting elements by ID,TAG also CreateElement method*/
formTagEvent.getById = function (id) {
    return document.getElementById(id)
};
formTagEvent.createEl = function (el) {
    return document.createElement(el)
};
formTagEvent.getByTag = function (name) {
    return document.getElementsByTagName(name)
};

/* Method that will be loaded country data like [id] and [name]*/
formTagEvent.loadCountriesList = function () {

    /* Injection tag <script> to DOM for cross-domain request ability*/
    var script = this.createEl("script");
    script.src = "http://www.ringcentral.com/api/index.php?cmd=getCountries&typeResponse=json&"
            + "callback=formTagEvent.getCountriesCB";
    script.type = "text/javascript";
    script.id = "cities";
    this.getByTag("head")[0].appendChild(script);
    formTagEvent.loaderActive();
...
/* Triggers for #loader element - hide/show when onclick button*/
formTagEvent.loaderActive = function () {
    this.getById('loader').style.display = 'block';
};
formTagEvent.loaderHide = function () {
    this.getById('loader').style.display = 'none';
};

/* Methods that will be passed as callback when the script tag dynamic injection */
formTagEvent.getCountriesCB = function (res) {
    formTagEvent.loaderHide();
    /* Object context */
    var self = this;
    if (res.status.code == 0 && res.status.success) {
        // Add each item [...{result:obj}] ] into countries[id:{obj}
        res.result.forEach(function (el) {
            self.countries[el.id] = el;
        });

        // Create <select> and append into options like -- <option id="country.id">country.name
...
formTagEvent.getCountryItemCB = function (res) {    
    formTagEvent.loaderHide();
...
formTagEvent.showTable = function (rateTypesList ) {
    var tbody = "";
    ...
    if(tbody){
        ...
        table += tbody;
        table += "</table>"
        this.getById('result').innerHTML = table;
    }

    //  If (tbody += tr;) -- is not filled then TBODY = false
    else {
        // No Table
        this.getById('result').innerHTML = '';
        alert(this.countries[this.selectCountry]['name'] + " has no rates");
    }

};

/* Gets all rates(areaCode, phonePart, rate, type) on a specific country*/
formTagEvent.getCountryInfo = function (e) {
    e.preventDefault();
    ...
    if (!selectInd) alert("Please! Select country item");
        ...
        script.src = "http://www.ringcentral.com/api/index.php?cmd=getInternationalRates&" +
                ...
                "param[internationalRatesRequest][countryId]=" + formTagEvent.selectCountry +
                "&param[internationalRatesRequest][tierId]=3311&" +
                "typeResponse=json&callback=formTagEvent.getCountryItemCB";
...
window.onload = function () {
    formTagEvent.loadCountriesList();
    formTagEvent.getById('search')
        .addEventListener('click', function (e) {
            return formTagEvent.getCountryInfo(e)
        });
};
